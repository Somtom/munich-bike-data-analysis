...
We are going to answer questions such as:

* [Can we create a map of all counting stations?](#station_map)
...
`.csv` format, the importing happens in two steps:
...
The following chunks will import the data for the years 2017 and 2018 (until end of July):
...
To prepare the data for the analysis we perform the following steps: 
...
bike_cnt_day %>% 
  ...
  filter(na_count > 0) %>% knitr::kable(digits = 2)
```
...
bind_rows(
  ...
  c(
    ...
    sd = sd(bike_cnt_15_min$gesamt)))  %>% knitr::kable(digits = 2)
```
...
station_summary_15_min  %>% knitr::kable(digits = 2)
```
...
station_summary_day  %>% knitr::kable(digits = 2)
```
...
wrong_way_drivers  %>% knitr::kable(digits = 2)
```
...
top_10_15_min  %>% 
  select(datum, gesamt, uhrzeit_start, zaehlstelle) %>% knitr::kable(digits = 2)
```
...
minute counts to also be listed in the daily count top 10 unless the high counts were due to
...
We then will have a closer look into the **"Highest 25%"** group and the weather parameters 
...
```{r temperature_and_weather_distribution_for_maximum_days, fig.width=10, fig.height=8, echo=FALSE}
...
bike_cnt_day_july_scaled %>% 
  ...
  filter(cat == "Highest 25%\n(q3-max]") %>% mutate(metric = plyr::revalue(metric, 
                                c("bewoelkung" = "Cloudiness [%]",
                                  ...
                                  "niederschlag" = "Precipitation  [ml]",
                                  "sonnenstunden" = "Hours of Sunshine [h]"))) %>% 
  select(-na, -cat)  %>% knitr::kable(digits = 2)
...
```{r 15_min_plot_weekend_week_per_station, fig.height=15, fig.width=13, echo=FALSE}
...
To create a really basic model of the commuting traffic, we make the following assumption:
...
```{r morning_and_afternoon_commuting_counts}
 bike_cnt_15_min %>% 
  ...
  filter(weekend == "Week") %>% 
  group_by(peak) %>% 
  summarise(bike_count = sum(median))  %>% knitr::kable(digits = 2)
```
...
Before I finish, I want to thank the Munich Open Data Team - especially Frank Börger and his coworker
Lukas Raffl - for their
great support and for providing me with inspirations and additional data.
Also, great thanks to my friends and colleagues Ryan, Stephie, Flo and my girlfriend Samira for proof reading!
...
you to also go out and ride your bike. Maybe you are living in Munich and want to add a few counts 
